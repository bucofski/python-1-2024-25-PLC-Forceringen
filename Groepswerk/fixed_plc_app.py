...
def run_head_and_capture_output(config_obj, selected_host_value):
    """Run head operations and capture output, handling async properly"""
    buffer = io.StringIO()
    old_stdout = sys.stdout
    old_stderr = sys.stderr
    sys.stdout = sys.stderr = buffer
    
    def run_head_operation():
        """Inner function to run the head operations in a separate thread with event loop"""
            ...
            try:
                if selected_host_value == "all":
                    ...
                    for host in config_obj.get_sftp_hosts():
                        ...
                        print(f"=== {host_name} ===")
                        try:
                            # Call the head function and await if it's a coroutine
                            result = head.run_main_with_host(config_obj, host_name)
                            if asyncio.iscoroutine(result):
                                await result
                            elif hasattr(result, '__await__'):
                                await result
                        except Exception as host_error:
                            print(f"Error processing {host_name}: {host_error}")
                        print()
                else:
                    try:
                        # Call the head function and await if it's a coroutine
                        result = head.run_main_with_host(config_obj, selected_host_value)
                        if asyncio.iscoroutine(result):
                            await result
                        elif hasattr(result, '__await__'):
                            await result
                    except Exception as host_error:
                        print(f"Error processing {selected_host_value}: {host_error}")
    ...
    try:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(run_head_operation)
...
def server(inputs, outputs, session):
    ...
    # Fixed async handling for resource clicks
    @reactive.effect
    def handle_resource_clicks():
        def process_resource_click():
                    ...
                    if hasattr(inputs, btn_id):
                        ...
                        if btn_input() > 0:
                            ...
                            def run_fetch():
                                ...
                                asyncio.set_event_loop(loop)
                                try:
                                    loop.run_until_complete(fetch_resource_bits(hostname, resource))
                                finally:
                                    loop.close()
        ...
        process_resource_click()

    async def fetch_resource_bits(hostname, resource):
        """Separate async function to fetch resource bits"""
        try:
            repo = PLCBitRepositoryAsync(config_loader)
            results = await repo.fetch_plc_bits(hostname, resource_name=resource)
            print("Results from plc_bits view:")
            for row in results:
                print(row)
        except Exception as e:
            print(f"Error fetching resource bits: {e}")

    # Fixed async handling for PLC clicks
    @reactive.effect
    def handle_plc_clicks():
        def process_plc_click():
            sftp_hosts = config.get('sftp_hosts', [])
                ...
                if hasattr(inputs, btn_id):
                    ...
                    if btn_input() > 0:
                        ...
                        def run_fetch():
                            ...
                            asyncio.set_event_loop(loop)
                            try:
                                loop.run_until_complete(fetch_plc_bits(hostname))
                            finally:
                                loop.close()
    ...
    async def fetch_plc_bits(hostname):
        """Separate async function to fetch PLC bits"""
        try:
            repo = PLCBitRepositoryAsync(config_loader)
            results = await repo.fetch_plc_bits(hostname)
            print("Results for PLC:", hostname)
            for row in results:
                print(row)
        except Exception as e:
            print(f"Error fetching PLC bits: {e}")
    ...
    def save_yaml_config():
        ...
        def process_save():
            try:
                ...
                # Step 4: Synchronize with database (in separate thread)
                def run_sync():
                    loop = asyncio.new_event_loop()
                    asyncio.set_event_loop(loop)
                    try:
                        loop.run_until_complete(sync_with_database())
                    finally:
                        loop.close()
